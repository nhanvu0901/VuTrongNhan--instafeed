...
class MainController(http.Controller):
    ...
    def main(self, **kw):
        ...
        shopify_app_exist = request.env['shopify.store'].sudo().search(
            ['&', ('admin', '=', current_user.id), ('is_delete', '=', False)], limit=1)
        ...
        if current_user and shopify_app_exist:
            ...
            data_transfer['flag'] = json.dumps({"status": "Not user"})
        return request.render("shopify_mint.shopify_mint_template", data_transfer)
            ...
            if data_username:
                ...
                if not instagram_user_exist and shopify_shop_exist:
                    ...
                    if shopify_shop_exist and instagram_user_exist_system:
                        ...
                        return {"status": "success", "message": "Instagram user already been used"}
                    ...
                    if instagram_user_exist.admin == shopify_shop_exist.user:  # if the instagram already use by 1 shop
                        ...
                        return {"status": "error", "message": "Instagram user already been used"}
                else:
                    return {"status": "Shop not exist", "message": "Shop not exist"}
