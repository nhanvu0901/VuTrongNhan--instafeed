...
class Instagram_User(models.Model):
                ...
                if response_media.ok:
                    ...
                    for data in data_media.get('data'):
                        ...
                        post = request.env['post.private'].sudo().search(
                            [('post_id', '=', json.loads(response_url.text).get('id'))], limit=1)
                        post.save_post(post, response_url)
    ...
    def refresh_long_live_tk(self):
        ...
        for instagram_user in self.env['instagram.user'].sudo().search([]):
            response_refresh_tk = instagram.refresh_long_live_token(instagram_user.ins_access_token)
    ...
    def cr_update_instagram_media(self):
        for instagram_user in self.env['instagram.user'].sudo().search([]):
            instagram_user.fetch_instagram_media()
